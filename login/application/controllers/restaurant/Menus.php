...
class Menus extends MY_Controller
{
 ...
 print_r($menus);die;*/
    ...
    public function index()
    {
        ...
        $data['head'] = 'Menus';
        $data['menu_limit'] = ($this->Menus_model->sql_select(TBL_MENUS, 'id', ['where' => ['is_deleted' => 0, 'restaurant_id' => $this->restaurant_id]], ['count' => true]));
       ...
       // print_r( $data['menus'] );die;
        ...
        $this->template->load('default', 'Backend/restaurant/menus/index', $data);
    }
    ...
    {
        $final['recordsFiltered'] = $final['recordsTotal'] = $this->Menus_model->get_menus('count');
        $final['redraw'] = 1;
        $menus = $this->Menus_model->get_menus('result');
        //print_r( $menus);die;
    ...
    {
       /*  $menuadd = $this->Menus_model->get_menu_all(['is_deleted' => 0]);
        print_r( $menuadd );die;*/
    ...
    public function edit($id)
    {
        ...
        if (is_numeric($id)) {
            $menu = $this->Menus_model->get_menu_detail(['id' => $id, 'is_deleted' => 0]);
            if ($menu) {
                $data['menu'] = $menu;
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $this->Menus_model->common_insert_update('update', TBL_MENUS, $dataArr, ['id' => $id]);
    ...
    public function delete($id = null)
    {
        $id = base64_decode($id);
        if (is_numeric($id)) {
            $menu = $this->Menus_model->get_menu_detail(['id' => $id], 'id,title');
            if ($menu) {
                $update_array = array(
                    'is_deleted' => 1,
    ...
    public function change_feedback()
    {
     ...
     public function change_webmenu()
    {
        $id = $this->input->post('id');
        if (is_numeric($id)) {
            $menu = $this->Menus_model->get_menu_detail(['id' => $id], 'id,title,is_webmenu_active');
            $is_webmenu_active;
            $msg = array();
            if ($menu) {
                // if ($menu['is_webmenu_active'] == 1) {
                //     $is_webmenu_active = 0;
                //     $msg['status'] = 1;
                //     $msg['msg'] = htmlentities($menu['title']) . ' has been disable  successfully!';
                // } else {
                ...
                $is_webmenu_active = 1;
                    $msg['status'] = 0;
                    $msg['msg'] = htmlentities($menu['title']) . ' has been set successfully!';
                $update_array = array(
                    'is_webmenu_active' => $is_webmenu_active,
                    'updated_at' => date('Y-m-d H:i:s')
                );
                $update_array1 = array(
                    'is_webmenu_active' => 0 
                );
                $this->Menus_model->common_insert_update('update', TBL_MENUS, $update_array1, ['restaurant_id' => $this->session->userdata('login_user')['id']]);

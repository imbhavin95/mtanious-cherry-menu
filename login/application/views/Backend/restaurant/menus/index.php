...
?>
    ...
    <div class="panel panel-default">
        ...
        <?php if ($menu_limit < $this->session->userdata('login_user')['menus_limit'])  {?>
            <a href="<?php echo base_url('restaurant/menus/add'); ?>" class="btn btn-default">Add New</a>
        <?php }?>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(function () {
            $('#menus').dataTable({
                ...
                order: [[6, "desc"]],
                ajax: '<?php echo base_url("restaurant/menus/get_menus"); ?>',
                columns: [
                    {
                        data: "sr_no",
                        visible: true,
                    ...
                    {
                        data: "is_webmenu_active",
                        visible: true,
                        searchable: false,
                        sortable: false,
                        render: function (data, type, full, meta) {
                            var checked_box = ''
                            if (full.is_webmenu_active == 1) {
                                checked_box = 'checked';
                            }
                            // class="checkbox margin-t-0"
                            var status =  '<div  style="text-align: center;"><input id="webmenu1'+ full.id +'" name="webmenu"   class="webmenu" data-id="'+ full.id +'" type="radio"' + checked_box + ' ><label for="webmenu'+ full.id +'"></label></div>';
                            return status;
                        }
                    },
                    {
                        ...
                        render: function (data, type, full, meta) {
                           ...
                           /* console.log( ' full='+full);
                            ...
                            var deleteurl = '<?php echo base_url(); ?>';
                                ...
                                action +='<a class="icons-design" href="restaurant/menus/edit/' + btoa(full.id) + '"><img src="https://www.cherrymenu.com/login/public/edit-change-pencil.svg"></a>&nbsp;'
                                ...
                                if(menu_id != full.id)
                                {
                                action +='<a class="icons-design" href="'+ deleteurl +'restaurant/menus/delete/'+ btoa(full.id) + '" onclick="return confirm_alert(this)"><img src="https://www.cherrymenu.com/login/public/Trashcan.svg"></a>&nbsp;'
                                }
    ...
    $(document).on('click', '.feedback', function () {
     ...
     $(document).on('click', '.webmenu', function () {
        $('#menu_disable').hide();
        $('#menu_enable').hide();
        var menu_id = $(this).attr('data-id');
        $.ajax({
            url: '<?php echo base_url("restaurant/menus/change_webmenu"); ?>',
            type: "POST",
            data: {id: menu_id},
            success: function (data) {
                if(data.status == 1)

...
class Users_model extends MY_Model
{
    ...
    public function get_users($type = 'result')
    {
        ...
        if($this->session->userdata('login_user')['role'] === ADMIN)
        {
            ...
            $this->db->join(TBL_SETTINGS . ' as ts', 'ts.user_id=users.id', 'left');
            $this->db->where(['users.id!=' => $this->session->userdata('login_user')['id'], 'users.role' => RESTAURANT,'users.is_deleted' => 0]);
            $this->db->order_by('users.'.$columns[$this->input->get('order')[0]['column']], $this->input->get('order')[0]['dir']);
        }
        else
        {
